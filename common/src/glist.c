...
#define _g_list_alloc()         xmalloc (sizeof(GList))
#define _g_list_alloc0()        xmalloc (sizeof(GList))
#define _g_list_free1(list)     xfree (list)
...
{
    GList * psList =  (GList*)_g_list_alloc0 ();
    if(!psList)
    {
        return NULL;
    }
...
void g_list_free_with_notify( GList * list,GDestroyNotify paNotify )
{
    GList * sCurrent;
    while(list)
    {
...
GList* g_list_insert_before (GList   *list, GList   *sibling, gpointer data)
{
    ...
    else
    {
        ...
        last->next = (GList*)_g_list_alloc ();
        g_return_val_if_fail(last->next,NULL);
        last->next->data = data;
        last->next->prev = last;
        last->next->next = NULL;
...
static __inline GList* _g_list_remove_link (GList *list, GList *link)
{
    if (link)
    {
        if (link->prev)
 ...
 * g_list_foreach_link:
 * @list: a #GList
 * @func: the function to call with each link
 * @user_data: user data to pass to the function
 *
 * Calls a function for each link
 */
/**
 * GFunc:
 * @data: the link
 * @user_data: user data passed to GFunc
 **/
void g_list_foreach_link(GList *list, GFunc func, gpointer user_data)
{
    while (list)
    {
        GList *next = list->next;
...
{
    return g_list_insert_sorted_real (list, data, (GFunc) func);
}
...
GList* g_list_sort_real( GList *list, GFunc compare_func )
{
    GList *l1, *l2;
    ...
    return g_list_sort_merge (g_list_sort_real (list, compare_func),
